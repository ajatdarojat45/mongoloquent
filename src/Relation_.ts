...
export default class Relation extends Query {
   ...
   */
  private $options: IRelationOptions | null = null;
   ...
   */
  public with(relation: keyof Model, options: IRelationOptions = {}): this {
   ...
   */
  hasOne(model: Model, foreignKey: string, localKey: string = "_id") {
   ...
   */
  morphTo(model: typeof Model, name: string): typeof Model {
   ...
   * @return {Model} The target model.
   */
  morphMany(model: Model, name: string): Model {
   ...
   * @return {Model} The target model.
   */
  morphToMany(model: typeof Model, name: string): typeof Model {
   ...
   */
  morphedByMany(model: typeof Model, name: string): typeof Model {
   ...
   * @returns {void}
   */
  private setLookups(doc: Document): void {
   ...
   * @return {void}
   */
  private setAlias(name: string): void {
    this.$alias = name;
  }
   ...
   */
  public async attach(ids: string | string[] | ObjectId | ObjectId[]) {
   ...
   */
  public detach(ids: string | string[] | ObjectId | ObjectId[]) {
    const relationship = this.getRelationship();
   ...
   * @return {Promise<{message: string}>} The result of the attach operation.
   */
  private async attachBelongsToMany(
    ...
    query = {
      ...
      [relationship.relatedPivotKey]: relationship.model.$id,
    };

    objectIds.forEach((id) =>
      _payload.push({
        [relationship.relatedPivotKey]: id,
        [relationship.foreignPivotKey]: relationship.model.$id,
      })
    );
   ...
   * @return {Promise<{message: string}>} The result of the attach operation.
   */
  private async attachMorphToMany(
    ids: string | string[] | ObjectId | ObjectId[]
  ) {
    const relationship = this.getRelationship();
    ...
    const collection = Database.getCollection(relationship.morphCollectionName);
    const _payload: object[] = [];

    query = {
      [relationship.foreignKey]: { $in: ids },
      [relationship.morphId]: relationship.model.$id,
      [relationship.morphType]: relationship.model.constructor.name,
    };
   ...
   * @return {Promise<{message: string}>} The result of the detach operation.
   */
  public async detachBelongsToMany(
    ids: string | string[] | ObjectId | ObjectId[]
  ) {
    const relationship = this.getRelationship();
    if (relationship?.type !== IRelationTypes.belongsToMany) return null;
   ...
   * @return {Promise<{message: string}>} The result of the detach operation.
   */
  private async detachMorphToMany(
    ids: string | string[] | ObjectId | ObjectId[]
  ) {
    const relationship = this.getRelationship();
    if (relationship?.type !== IRelationTypes.morphToMany) return null;
    ...
    const query = {
      [relationship.foreignKey]: { $in: ids },
      [relationship.morphId]: relationship.model.$id,
      [relationship.morphType]: relationship.relatedModel.constructor.name,
    };
   ...
   */
  public sync(ids: string | string[] | ObjectId | ObjectId[]) {
    const relationship = this.getRelationship();
   ...
   * @return {Promise<{message: string}>} The result of the sync operation.
   */
  private async syncBelongsToMany(
    ids: string | string[] | ObjectId | ObjectId[]
  ) {
    const relationship = this.getRelationship();
    if (relationship?.type !== IRelationTypes.belongsToMany) return null;
