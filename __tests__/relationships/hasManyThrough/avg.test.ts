...
describe("avg method", () => {
  it("return avg document", async () => {
    ...
    await Deployment.insertMany([
      ...
      { commit_hash: "789", environmentId: environmentIds[2], count: 1 },
    ]);

    const application = await Application.find(applicationIds[0]);
    const deployments = await application.deployments().avg("count");

    expect(deployments).toEqual(expect.any(Number));
    expect(deployments).toBe(2.5);
  });
});
