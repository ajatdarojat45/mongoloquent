...
describe("max method", () => {
  describe("without soft delete", () => {
    ...
    it("return max of tags", async () => {
      ...
      const count = await post.tags().max("likes");
  ...
  describe("with soft delete", () => {
    ...
    it("return max of tags", async () => {
      ...
      const tagIds = await Tag.insertMany([
        { name: "Tag 1", active: true, likes: 10 },
        { name: "Tag 2", active: false, likes: 5 },
        { name: "Tag 3", active: true, likes: 20 },
      ]);
      ...
      const post2 = await Post.find(postIds[1]);
      await post2.tags().attach([tagIds[0], tagIds[1]]);
      const count = await post.tags().min("likes");
      expect(count).toBe(5);

      const tags = await Tag.withTrashed().max("likes");
      expect(tags).toBe(20);
    });
  });
});
